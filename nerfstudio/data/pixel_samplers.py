...
nonzero_idx_buffer = None
nonzero_idx_buffer_flag = False
nonzero_img_num = 0
...
def collate_image_dataset_batch(batch: Dict, num_rays_per_batch: int, keep_full_image: bool = False):
    ...
    # dont use torch.nonzero since it is extremely slow
    if "mask" in batch:
        if globals()["nonzero_idx_buffer_flag"] and batch['image'].shape[0] == globals()["nonzero_img_num"]:
            nonzero_indices = globals()["nonzero_idx_buffer"]
        else:
            nonzero_indices = torch.nonzero(batch["mask"][..., 0].to(device), as_tuple=False)
            globals()["nonzero_idx_buffer"] = nonzero_indices
            globals()["nonzero_img_num"] = batch['image'].shape[0]
            globals()["nonzero_idx_buffer_flag"] = True
    ...
    try:
        ...
        #  dict_keys(['image_idx', 'image', 'confmask', 'filtered_raylen', 'mask']) batch['image_idx'][c, y, x]
