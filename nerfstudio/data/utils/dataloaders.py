...
class CacheDataloader(DataLoader):
    ...
    def _get_collated_batch(self):
        ...
        try:
            collated_batch = get_dict_to_torch(collated_batch, device=self.device, exclude=["image",'mask','confmask','filtered_raylen']) # =["image", 'mask','confmask', 'filtered_raylen'])
            print('collated all')
        except:
            collated_batch = get_dict_to_torch(collated_batch, device=self.device, exclude=["image"]) # =["image", 'mask','confmask', 'filtered_raylen'])
            print('collated image only')
