...
class Trainer:
    ...
    def train(self) -> None:
        ...
        with TimeWriter(writer, EventName.TOTAL_TRAIN_TIME):
            ...
            step = 0
            import shutil
            shutil.copytree('../nerfstudio', str(self.base_dir)+'/code')

            for step in range(self._start_step, self._start_step + num_iterations):
                if step == megasurf_conf['init_radiance_end'] + 1 and self.megasurf_flag:
                    step = 0
                    self.optimizers = setup_optimizers(self.config, self.pipeline.get_param_groups())
                ...
                if step_check(step, self.config.trainer.steps_per_save):
                    ...
                    # extract_script = os.getcwd() + '/extract.sh'
                    # py_script = os.getcwd() + '/extract_mesh.py'
                    # cuid = str(4)
                    # modelpath = os.path.join(os.getcwd(),os.path.dirname(self.checkpoint_dir),'config.yml')
                    # outpath = os.path.join(os.getcwd(),os.path.dirname(self.checkpoint_dir),f'{step}.ply')
                    #
                    # os.system(f'bash extract.sh {modelpath} {outpath}')
